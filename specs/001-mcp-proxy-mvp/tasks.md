...
- [ ] T014 [US1] Implement `PolicyEngine::evaluate` in `src/policy/engine.rs`: accept `&PolicyRequest` (public type — `Tool { name, arguments }` or `Resource { uri }`); iterate rules top-to-bottom, match `tool` rules against `PolicyRequest::Tool` by tool name (exact or `*`), match `resource` rules against `PolicyRequest::Resource.uri` via compiled `GlobSet`, apply `path_prefix` constraint check on arguments, return first-matching `PolicyDecision`; return `DenyByDefault` if no rule matches; `proxy::intercept()` converts internal `McpRequest` → `PolicyRequest` before calling evaluate (FR-001–FR-008b)
