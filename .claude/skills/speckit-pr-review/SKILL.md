...
description: "Spec-grounded review of an open pull request. Reads the active feature's spec, plan, tasks, contracts, and constitution; reads the branch diff via git; runs detection passes for FR coverage, contract conformance, SC evidence, constitution compliance, public API surface, scope drift, and test gaps; posts a structured report as a PR comment via the detected git provider."
argument-hint: "Optional PR/MR number (auto-detected from current branch if omitted), and optional --no-comment flag"
compatibility: "Requires spec-kit project structure with .specify/ directory and git. A provider CLI (gh for GitHub, glab for GitLab, bb for Bitbucket) is auto-detected at runtime and is only required when posting a comment (skippable with --no-comment)."
metadata:
  author: "Odyssey LLC"
  source: ".claude/skills/speckit-pr-review/SKILL.md"
user-invocable: true
...
```markdown
<!-- speckit-pr-review-marker -->
## /speckit-pr-review report
...
---
*Generated by `.claude/skills/speckit-pr-review` — re-run to refresh.*
```
...
The leading HTML comment marker `<!-- speckit-pr-review-marker -->` makes future versions of this skill identify and (optionally, in a later iteration) edit the existing comment instead of stacking duplicates. v1 always posts a new comment.
